...
const char * const BITCOIN_CONF_FILENAME = "cslabcoin.conf";
const char * const BITCOIN_PID_FILENAME = "cslabcoin.pid";
...
#else
    const char* pszModule = "cslabcoin";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "CSLabCoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/CSLabCoin";
#else
    // Unix
    return pathRet / ".cslabcoin";
#endif
#endif
}
